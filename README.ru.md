...
Для обеспечения нашего рабочего процесса и предотвращения распространенных ошибок, этот репозиторий использует современный механизм GitHub — **"Rulesets"** (Наборы правил), так как он гибче старых "Branch protection rules". Этот гайд проведет вас через настройку.

#### Пошаговая инструкция по настройке
Вам нужно будет создать **два** набора правил:
*  **Строгий набор:** для веток `main` и `stg`, где приоритет — максимальная стабильность.
*  **Гибкий набор:** для ветки `dev`, где приоритет — скорость разработки.

##### **Шаг 1: Перейдите в настройки**
В вашем репозитории откройте `Settings` (Настройки) -> `Branches` (Ветки) -> `Rulesets` (Наборы правил).

##### **Шаг 2: Создание строгого набора правил для `main` и `stg`**
1. Нажмите **"New branch ruleset"** (Новый набор правил).
2. **Ruleset name:** Введите `Production & Staging Rules`.
3. **Enforcement status:** Оставьте `Active`.
4. **Target branches (Целевые ветки):**
  *  Нажмите **"Add target"**.
  *  В выпадающем меню выберите **"Include by pattern"** (Включить по шаблону).
  *  В поле введите `main,stg` и нажмите `Add`. Это применит правила к обеим веткам.
5. **Настройте правила:** В секции **"Branch rules"** поставьте галочки и настройте параметры так, как указано в колонке **"Настройка для `main` и `stg`"** в таблице ниже.
6. Нажмите **"Create"** (Создать).

##### **Шаг 3: Создание гибкого набора правил для `dev`**
1. Снова нажмите **"New branch ruleset"**.
2. **Ruleset name:** Введите `Development Rules`.
3. **Enforcement status:** Оставьте `Active`.
4. **Target branches (Целевые ветки):**
  *  Нажмите **"Add target"**.
  *  Выберите **"Include by pattern"**.
  *  В поле введите `dev` и нажмите `Add`.
5. **Настройте правила:** Поставьте галочки и настройте параметры так, как указано в колонке **"Настройка для `dev`"** в таблице ниже.
6. Нажмите **"Create"** (Создать).

---
#### Детальный разбор правил и настроек

| Опция в UI (сверху вниз) | Настройка для `main` и `stg` | Настройка для `dev` | Что это делает и зачем мы принимаем такое решение |
| :--- | :--- | :--- | :--- |
| **Ruleset Name** | Ввести: `Production & Staging Rules` | Ввести: `Development Rules` | **Что делает:** Задает имя для набора правил. <br>**Почему так:** Простое и понятное имя помогает ориентироваться в настройках. |
| **Enforcement status** | Выбрать: `Active` | Выбрать: `Active` | **Что делает:** Включает или отключает набор правил. <br>**Почему нужно:** Правила должны быть активны, чтобы работать. |
| **Target branches** | 1. Нажать `Add target`<br>2. Выбрать `Include by pattern`<br>3. Ввести: `main,stg` | 1. Нажать `Add target`<br>2. Выбрать `Include by pattern`<br>3. Ввести: `dev` | **Что делает:** Определяет, к каким веткам применяется этот набор правил. <br>**Почему так:** Создаем два разных набора правил для стабильных веток и для ветки разработки. |
| **--- Секция Branch rules ---** | | | --- |
| `Restrict creations` | ❌ **Не включать** | ❌ **Не включать** | **Что делает:** Запрещает создание веток с таким именем. <br>**Почему не нужно:** Эти ветки у нас уже существуют. |
| `Restrict updates` | ❌ **Не включать** | ❌ **Не включать** | **Что делает:** Полностью блокирует любые изменения в ветке. <br>**Почему не нужно:** Это остановит весь рабочий процесс. |
| `Restrict deletions` | ✅ **Включить** | ✅ **Включить** | **Что делает:** Запрещает удаление ветки. <br>**Почему нужно:** **Критически важно.** Защищает ключевые ветки от случайного удаления. |
| `Require linear history` | ❌ **Не включать** | ❌ **Не включать** | **Что делает:** Принуждает к истории без merge-коммитов. <br>**Почему не нужно:** Мы управляем этим через `Allowed merge methods`, что более гибко. |
| `Require deployments to succeed` | ❌ **Не включать** | ❌ **Не включать** | **Что делает:** Требует успешного деплоя перед слиянием. <br>**Почему не нужно:** Избыточно для PoC. |
| `Require signed commits` | ❌ **Не включать** | ❌ **Не включать** | **Что делает:** Требует, чтобы коммиты были подписаны GPG-ключом. <br>**Почему не нужно:** Лишнее усложнение для PoC. |
| **`Require a pull request before merging`** | ✅ **Включить** | ✅ **Включить** | **Что делает:** Запрещает пушить напрямую в ветку. <br>**Почему нужно:** **Фундаментальное правило.** Обеспечивает код-ревью и контроль. |
| ↳ `Required approvals` | Ввести: `0` | Ввести: `0` | **Что делает:** Устанавливает число обязательных подтверждений. <br>**Почему так:** Для **PoC** ставим `0` везде для максимальной скорости. **Рекомендация для Production:** `1` или `2` для `main`/`stg`, и `1` для `dev`. |
| ↳ `Dismiss stale pull request approvals...` | ✅ **Включить** | ✅ **Включить** | **Что делает:** Сбрасывает апрув, если в PR запушили новые коммиты. <br>**Почему нужно:** **Критически важно!** Гарантирует, что ревьюеры одобряют **финальную** версию кода. |
| ↳ `Require review from Code Owners` | ❌ **Не включать** | ❌ **Не включать** | **Что делает:** Требует апрув от владельцев кода из `CODEOWNERS`. <br>**Почему не нужно:** В PoC не используем. **Рекомендация для Production:** Отличный паттерн для назначения ответственных. |
| ↳ `Require approval of the most recent...` | ❌ **Не включать** | ❌ **Не включать** | **Что делает:** Требует апрув на самый последний пуш от кого-то, кроме автора. <br>**Почему так:** Как вы верно заметили, для PoC это избыточно. В Production для `main`/`stg` это также может быть лишним, если промоушен делает доверенный релиз-менеджер. Отключаем везде. |
| ↳ `Require conversation resolution...` | ✅ **Включить** | ✅ **Включить** | **Что делает:** Блокирует слияние, если есть неразрешенные комментарии. <br>**Почему нужно:** Все вопросы должны быть закрыты до слияния. Это правило одинаково важно для всех ключевых веток. |
| ↳ `Allowed merge methods` | Выбрать только `Merge commit` | ✅ **Разрешить все** (`Merge`, `Squash`, `Rebase`) | **Что делает:** Ограничивает способы слияния PR. <br>**Почему так:** Для `main`/`stg` — строгая, аудируемая история. Для `dev` — максимальная гибкость, т.к. ответственность за качество коммитов лежит на локальных хуках. |
| **`Require status checks to pass`** | ❌ **Не включать** | ❌ **Не включать** | **Что делает:** Требует успешного прохождения CI-проверок. <br>**Почему так:** В рамках **PoC** отключаем везде для скорости. **Рекомендация для Production:** Включить для всех веток и добавить обязательные проверки (тесты, линтеры). |
| ↳ `Require branches to be up to date...` | ✅ **Включить** | ✅ **Включить** | **Что делает:** Требует обновить PR, если целевая ветка изменилась. <br>**Почему нужно:** **Важно для всех веток.** Это предотвращает "слепые" слияния и конфликты. **Примечание:** Обновление ветки должно производиться разработчиком вручную (`rebase`/`merge`), а не через кнопку в UI, для полного контроля. |
| **`Block force pushes`** | ✅ **Включить** | ✅ **Включить** | **Что делает:** Запрещает `git push --force`. <br>**Почему нужно:** **Обязательное правило.** Защищает общую историю от перезаписи. |
| **`Require code scanning results`** | ❌ **Не включать** | ❌ **Не включать** | **Что делает:** Требует успешного прохождения сканирования безопасности. <br>**Почему не нужно:** Продвинутая функция, не нужна для PoC. |
| **`Automatically request Copilot code review`** | ❌ **Не включать** | ❌ **Не включать** | **Что делает:** Автоматически запрашивает ревью у Copilot. <br>**Почему не нужно:** Экспериментальная функция, не нужна для PoC. |
...
**Проблема:** Стандартный `secrets.GITHUB_TOKEN`, генерируемый для каждого запуска workflow, обладает ограниченными правами и, что самое главное, не может запускать другие workflows. Это делает невозможным наш двухфазный процесс, где мерж Release PR должен триггерить фазу финализации релиза.
**Решение:** Мы должны использовать выделенное удостоверение (identity) с расширенными правами. Наиболее безопасным и рекомендуемым способом является создание **GitHub App**. В отличие от токенов пользователей (PAT), этот метод использует короткоживущие токены установки, что значительно повышает безопасность.
**Требуемая настройка (одноразовое действие администратора):**

1.  **Создайте GitHub App:**
    *   Перейдите в `Settings > Developer settings > GitHub Apps` и создайте новое приложение.
    *   Дайте ему необходимые разрешения (**Permissions**):
        *   `Repository permissions > Contents`: `Read and write` (для изменения `CHANGELOG.md` и манифеста).
        *   `Repository permissions > Pull requests`: `Read and write` (для создания и мержа Release PR).
2.  **Установите приложение:** Установите созданное приложение в ваш репозиторий.
3.  **Сгенерируйте Private Key:** На странице приложения сгенерируйте и скачайте приватный ключ (`.pem` файл).
4.  **Сохраните секреты:** Добавьте в секреты репозитория (`Settings > Secrets and variables > Actions`) две переменные:
    *   `RELEASE_APP_ID`: ID вашего GitHub App.
    *   `RELEASE_APP_PRIVATE_KEY`: Содержимое вашего `.pem` файла.
**Интеграция в Workflow (`release-please.yml`):**

Наш workflow теперь использует эти секреты для генерации временного токена перед запуском `release-please`. Это делается с помощью экшена `tibdex/github-app-token`.
...
jobs:
  release-please:
    ...
    steps:
      # 1. Генерация временного токена от имени GitHub App
      ...
      # 2. Запуск release-please с использованием сгенерированного токена
